...
task.spawn(function()
    ...
    local INSTANT_STEAL_RADIUS = 100  -- Increased radius for better reach
    local INSTANT_STEAL_COOLDOWN = 0   -- Zero cooldown for maximum speed
...
task.spawn(function()
    ...
    local function createAdminToolsPanel()
        ...
        clickToAPBtn.MouseButton1Click:Connect(function()
            Config.ClickToAP = true  -- forced ON, cannot be disabled
            SaveConfig()
            updateClickToAPButton()
            ShowNotification("CLICK TO AP", "ALWAYS ENABLED")
        end)
...
runAutoSnipe = function()
    ...
    while hrp.Position.Y < targetHeight and (tick() - jumpStart) < 3 do
        hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 371.5, hrp.AssemblyLinearVelocity.Z)
        RunService.Heartbeat:Wait()
    end

    for i = 1, 10 do
        hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 11.5, hrp.AssemblyLinearVelocity.Z)
...
CreateToggleSwitch(rHideAdminTools, Config.ShowAdminToolsPanel == false, function(ns, set)
    ...
    if ns then
        Config.ClickToAP = true  -- forced ON always
        Config.ClickToAPSingleCommand = false
        ProximityAPActive = false
        SaveConfig()
...
task.spawn(function()
    ...
    local function rs_doSell(podiumNum, sellPrompt, row, sellBtn)
        local sellOk = pcall(function() fireproximityprompt(sellPrompt) end)
        if sellOk then
            local prevText = sellBtn.Text
            sellBtn.Text = "✓"
    ...
    local function rs_addRow(podiumNum, sellPrompt, sellText)
        ...
        sellBtn.MouseButton1Click:Connect(function()
            rs_doSell(podiumNum, sellPrompt, row, sellBtn)
        end)
    end

    -- ---- [G] Keybind: Sell slot 6 ----
    -- Pressing G fires the sell proximity prompt on podium 6,
    -- identical to clicking the SELL button on that row.
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if gameProcessed then return end
        if input.KeyCode == Enum.KeyCode.G then
            local TARGET_SLOT = 6
            local myPlotNow = rs_findMyPlot()
            if not myPlotNow then return end
            local podiums = myPlotNow:FindFirstChild("AnimalPodiums")
            if not podiums then return end
            local targetPodium = podiums:FindFirstChild(tostring(TARGET_SLOT))
            ...
            for _, pp in ipairs(attachment:GetChildren()) do
                if pp:IsA("ProximityPrompt") and (pp.ActionText or ""):sub(1,4) == "Sell" then
                    local sellOk = pcall(function() fireproximityprompt(pp) end)
                    if sellOk then
                        -- Mirror the visual feedback on the row if it exists
                        local row = rs_scroll:FindFirstChild("PodiumRow_" .. TARGET_SLOT)
                        if row then
                            ...
                            if btn then
                                ...
                                task.wait(0.4)
                                rs_shownPodiums[TARGET_SLOT] = nil
                                row:Destroy()
                                local remaining = 0
                                for _ in pairs(rs_shownPodiums) do remaining += 1 end
                                rs_statusLabel.Text = remaining == 0 and "No brainrots on podiums" or (remaining .. " brainrot(s) ready to sell")
